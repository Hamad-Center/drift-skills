---
name: drift-status
description: Quick read-only status check of the active drift session — shows changes, commits, and session history without writing any files.
---
...
```
No drift session found. Start one with drift-start
```
And stop.
...
If `.drift/sessions.codex.json` exists, read it and show the last 5 entries:

```
Recent sessions (Codex):
...
If `.drift/sessions.cursor.json` exists, read it and show the last 5 entries:
...
If no session history files exist or all are empty, omit this section.
