---
name: drift-start
description: Start a drift session — record current git SHA and session metadata to .drift/config.json. Provide a ticket ID and optional planned work (e.g., BAS-11 implement user auth).
---
...
### Step 4: Parse arguments from the user's message

Parse the user's message to extract:
- **Ticket ID**: Match the pattern `[A-Za-z][A-Za-z0-9]+-\d+` (case-insensitive — e.g., PROJ-123, proj-123, Ab-1). Normalize to UPPERCASE. **Required** — if no ticket ID is found, ask the user to provide one before continuing. They cannot skip this.
- **Planned work**: Everything in the user's message that isn't the ticket ID or the "drift-start" trigger. Trim whitespace. This is optional.

Examples:
- `drift-start PROJ-123 implement user authentication` → ticket=`PROJ-123`, plannedWork=`implement user authentication`
- `drift-start proj-123 implement user authentication` → ticket=`PROJ-123`, plannedWork=`implement user authentication`
- `drift-start implement user authentication` → no ticket found → ask user for ticket ID
- `drift-start PROJ-123` → ticket=`PROJ-123`, plannedWork=null
- `drift-start` → no ticket found → ask user for ticket ID
...
- No `plannedWork` was parsed from the user's message
