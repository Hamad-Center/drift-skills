---
name: drift-sync
description: Deep multi-step analysis of all changes since session start — writes CONTEXT.md
...
Perform a deep, multi-step analysis of all code changes since the drift session started. This is the core skill that leverages Claude's full context window and multi-step reasoning — unlike the drift CLI which truncates diffs to 8,000 characters.
...
- `$ARGUMENTS` contains `--no-push` or `--no-jira` (the user wants to skip Jira integration)
...
### Phase 3 — Deep file analysis (Claude's key advantage)
...
3. Read the full current content of each remaining file using the Read tool
...
- Use Grep to find references to changed functions/classes in unchanged files
...
- TODOs found in the code (use Grep to find `TODO`, `FIXME`, `HACK` in changed files)
...
### Phase 6 — Write .drift/sessions.claude.json

Read `.drift/sessions.claude.json` if it exists (it's an array). Prepend a new entry to the beginning of the array:
...
- `$ARGUMENTS` contains `--no-push` or `--no-jira`
         ...
         "content": [
           {
             ...
             "content": [
               {
                 "type": "text",
                 "text": "Drift Session Summary (Claude analysis):\n\n<ticketComment text>"
               }
             ]
           }
         ]
