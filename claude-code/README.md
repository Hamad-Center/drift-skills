# Drift Skills for Claude Code

Track how your code drifts from plan. These skills run inside Claude Code as slash commands, connecting your git changes to Jira tickets with deep AI-powered analysis.
...
6. Writes `.drift/sessions.claude.json` history (max 50 entries)
...
- `.drift/sessions.claude.json` — structured session history
...
- Uncommitted changes
- Recent session history from Claude, Cursor, Codex, and CLI logs
...
| `sessions.claude.json` | `.drift/` | Prepended every sync, max 50 entries | Machine-readable JSON session history |
...
These skills are installed globally at `~/.claude/skills/`:
...
They work in any git repository. Invoke with slash commands:
...
## Compatibility with Cursor and Codex

Drift is also available as Cursor rules (`~/.cursor/rules/`) and Codex skills (`~/.agents/skills/`). All three versions share the same `.drift/` directory and `CONTEXT.md` output.
