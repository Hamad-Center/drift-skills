# Drift Rules for Cursor

Track how your code drifts from plan. These rules run inside Cursor as agent-mode instructions, connecting your git changes to Jira tickets with deep AI-powered analysis.
...
Deep multi-step analysis of everything that changed since the session started. This is the core rule.
...
6. Writes `.drift/sessions.cursor.json` history (max 50 entries)
...
**Key design decision:** Progress, drift, and next steps are evaluated ONLY against what is explicitly written in the ticket's acceptance criteria. The rule never assumes or infers requirements that aren't written.

**Files created/updated:**
- `CONTEXT.md` — full analysis in the repo root
- `<ticket>-report.md` — session report (e.g., `bas-10-report.md`)
- `.drift/sessions.cursor.json` — structured session history
...
- Uncommitted changes
- Recent session history from Cursor, Claude, Codex, and CLI logs
...
The rules connect to Jira in two directions:
...
| 6 | Interactive prompt | The rule asks during `drift-start` or `drift-sync` |
...
| `sessions.cursor.json` | `.drift/` | Prepended every sync, max 50 entries | Machine-readable JSON session history |
...
These rules are installed globally at `~/.cursor/rules/`:
...
They work in any git repository. In Cursor agent mode, type the rule name and Cursor matches it by description:
...
Drift is also available as Claude Code skills (`~/.claude/skills/`) and Codex skills (`~/.agents/skills/`). All three versions share the same `.drift/` directory and `CONTEXT.md` output.
